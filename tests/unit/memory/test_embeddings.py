...
def provider(mock_voyage_client):
    with patch("atlas.memory.embeddings.voyageai") as mock_voyageai:
        mock_voyageai.AsyncClient.return_value = mock_voyage_client
        p = EmbeddingProvider(api_key="test-key", model="voyage-3-lite")
        yield p


async def test_embed_returns_numpy_array(provider, mock_voyage_client):
...
async def test_embed_batch_returns_list_of_arrays(provider, mock_voyage_client):
...
async def test_embed_returns_none_on_api_error(provider, mock_voyage_client):
    mock_voyage_client.embed.side_effect = Exception("API timeout")
    result = await provider.embed("hello")
    assert result is None


async def test_embed_batch_returns_empty_on_api_error(provider, mock_voyage_client):
    mock_voyage_client.embed.side_effect = Exception("API timeout")
    results = await provider.embed_batch(["text"])
    assert results == []
